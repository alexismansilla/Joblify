                        ...
                        <div className="flex flex-col gap-4 mt-8">
                            {Object.entries(connectionTypesCount).map(([type, count]) => {
                                const percentage = stats.total_matches > 0
                                    ? Math.round((count / stats.total_matches) * 100)
                                    : 0
                                return (
                                    <div key={type} className="flex justify-between items-baseline border-b border-black/5 dark:border-white/5 pb-2 last:border-0 last:pb-0 group/stat">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-mono text-[10px] tracking-widest opacity-0 group-hover/stat:opacity-40 transition-opacity">({count})</span>
                                        </div>
                                ...
                                <tbody className="divide-y divide-black/10 dark:divide-white/10">
                                    {topEmpresas.map((user) => (
                                        <tr key={user.id} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                                            ...
                                            <td className="px-4 py-4 text-center w-1/4 align-top">
                                                ...
                                                <div className="flex flex-col gap-1.5">
                                                    {user.matches.length > 0 ? (
                                                        <>
                                                            ...
                                                            {user.match_count > 5 && (
                                                                <div className="text-center w-full py-3 border border-dashed border-black/20 dark:border-white/20 text-[10px] font-mono font-bold uppercase tracking-widest">
                                                                    + {user.match_count - 5} REGISTROS OCULTOS
                                                                </div>
                                                            )}
                                                        </>
                                                    ) : (
                                                        <div className="py-6 text-xs font-mono uppercase tracking-widest opacity-40">
